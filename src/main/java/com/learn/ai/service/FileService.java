...
public class FileService {
    ...
    public boolean save(String conversationId, Resource resource) {
        ...
        // 2.保存映射关系
        chatFiles.put(conversationId, filename);
        // 3.写入向量库
        writeToVectorStore(resource, conversationId);
        return true;
    }

    public Resource getFile(String conversationId) {
        return new FileSystemResource(chatFiles.getProperty(conversationId));
    }
    ...
    private void writeToVectorStore(Resource resource, String conversationId) {
        // 1.创建PDF的读取器
        ...
        log.info("准备写入向量库，conversationId: {}, 文档数量: {}", conversationId, documents.size());
        ...
        log.info("已写入向量库，conversationId: {}", conversationId);
    }
}